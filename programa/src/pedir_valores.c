...
int utn_verificar(char* mensaje, char* mensajeError, int reintentos)
{
	...
	if(mensaje != NULL && mensajeError != NULL && reintentos > -1)
	{
		do
		{
			...
			{
				if(!utn_esVerificable(buffer, sizeof(buffer)) && strlen(buffer) > 0)
				{
					retorno = 0;//si
					break;
				}
				...
				{
					if(utn_esVerificable(buffer, sizeof(buffer)) == 1 && strlen(buffer) > 0)
					{
						retorno = 1; //no
						break;
					}
...
int utn_pedirCadena(char cadena[], int longitud, char* mensaje, char* mensajeError, int reintentos)
{
	int retorno = -1;
	char buffer[500];
	if(cadena != NULL && mensaje != NULL &&
	   ...
	   && longitud > 0)
	{
		...
		{
			printf("%s", mensaje);
			if(!pedirCadena(buffer, sizeof(buffer)) && strlen(buffer) > 0)
			{
				strncpy(cadena, buffer, longitud);
				retorno = 0;
				break;
